  ...
  return (
    <div>
      <div>
        <p>Filtrer</p>
        <div>
          <span className={`${active == 'all' ? "active" : ''}`} onClick={(e) => handleCategory('all')}>Tous</span>
          {filter.map( (filter, index) => (
            <span key={index} className={`${active == filter ? "active" : ''}`} onClick={(e) => handleCategory({filter})}>{filter}</span>
          ))}
        </div>
      </div>
